...
* Ab ONTAP 9.10.1 `network interface` enthalten die CLI-Befehle einen `-rdma-protocols` Parameter für NFS over RDMA-Konfigurationen. Die Erstellung von Netzwerkschnittstellen für NFS über RDMA-Konfigurationen wird in System Manager ab ONTAP 9.12.1 unterstützt. Weitere Informationen finden Sie unter xref:https://docs.netapp.com/us-en/ontap/nfs-rdma/configure-lifs-task.html[Konfigurieren Sie LIFS für NFS über RDMA^].
...
+
link:https://docs.netapp.com/us-en/ontap/san-admin/asa-iscsi-lif-fo-task.html["LIF-Failover für ASA-Plattformen"^]
